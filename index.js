...
mongoose.connect('mongodb+srv://AQT_dev:aqctsvrm@act.io0on.mongodb.net/AQT_dev?retryWrites=true&w=majority',{ useNewUrlParser: true, useUnifiedTopology: true })
.then(()=>{
    /*
    ...
    while(j<subunits.length && subunits[j]!="-1"){
        var modelObj = new model_info({name:units[i],topic:subunits[j]}); 
        j++;

        //console.log("current"+i+" "+j);
        modelObj.save()
        .then((data)=>console.log(" "+i+" fine "+j))
